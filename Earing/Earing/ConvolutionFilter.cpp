...
void ConvolutionFilter::setNewIR(const IRF* irf) {
    irf_ = irf;
    if(window_.get_inputBlockSize() != 0)
        partitionedIR_.setNewIRF( irf_, window_.get_channelLength() );
}

void ConvolutionFilter::process(FelidadaeAudio::AudioInOutBuffers<float_type>& audio ) {
    ...
    if ( window_.get_inputBlockSize() != audio.channelLength_ || window_.get_numOfChannels() != audio.numOfChannels_) {
        ...
        //Create
        for(unsigned int iBuffer = 0; iBuffer < 4; ++iBuffer)
            *(buffers[iBuffer])   = (float_type*) malloc (sizeof(float_type) * /*>>>*/_2B * _C/*<<<*/);
        bufferFDL_R_= (float_type*) malloc(sizeof(float_type) * _2B * _C * _P );
        bufferFDL_I_= (float_type*) malloc(sizeof(float_type) * _2B * _C * _P );
        //.
        ...
        //Recreate plans
		fftPlan = new FourierTransformBox(_2B);
		//.
    }
	...
	fftPlan->execute(audio.numOfChannels_, bufferTransform_R_, bufferTransform_I_, FFT_FORWARD);
    //.
    ...
    //ifft
    fftPlan->execute(audio.numOfChannels_, bufferAccumulator_R_, bufferAccumulator_I_, FFT_INVERSE);
    //.
